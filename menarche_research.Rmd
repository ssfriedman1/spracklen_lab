...
```{r}
merged <- right_join(x = menarche_data, y = demo_data, by ="ID" )

merged <- right_join(x = merged, y = socio_data, by = "ID")

merged <- right_join(x = merged, y = race_data, by ="ID")

merged <- right_join(x = merged, y = preg_data, by = "ID")
```
...
```{r}
table1(~ AGE + MENARCHE + ETHNICITY + RACE + SMOKING + ALCOHOL| PREECLMP, data = merged)

table1(~ AGE + MENARCHE + ETHNICITY + RACE + SMOKING + ALCOHOL| HYPTPREG, data = merged)

table1(~ AGE + MENARCHE + ETHNICITY + RACE + SMOKING + ALCOHOL| BABYLT5LBS, data = merged)

table1(~ AGE + MENARCHE + ETHNICITY + RACE + SMOKING + ALCOHOL| BABYGT9LBS, data = merged)

table1(~ AGE + MENARCHE + ETHNICITY + RACE + SMOKING + ALCOHOL| DIABPREG, data = merged)

table1(~ AGE + MENARCHE + ETHNICITY + RACE + SMOKING + ALCOHOL| , data = merged)
```